...
bundle_root="$repo_root/deploy/pcsx2-sdl-r36s-diag"
...
binary_src="$build_root/bin/armsx2-sdl"
sdl3_src="$sdl3_build_root/libSDL3.so.0.2.6"
expected_sdl3_sha="7c8e71e6ccfd2673f1a9e3181d422d623fdb5773e9dece02474ee7c93be1d731"

require_file "$binary_src" "built armsx2-sdl"
require_file "$sdl3_src" "proven SDL3 shared library"
require_dir "$smoke_lib_root" "proven SDL3 Wayland runtime directory"
require_dir "$support_lib_root" "PCSX2 runtime support directory"

actual_sdl3_sha="$(sha256_of "$sdl3_src")"
if [[ "$actual_sdl3_sha" != "$expected_sdl3_sha" ]]; then
	die "unexpected SDL3 SHA256: ${actual_sdl3_sha} (expected ${expected_sdl3_sha})"
fi

rm -rf "$bundle_root"
mkdir -p "$bundle_root/bin" "$bundle_root/lib"

install -m 0755 "$binary_src" "$bundle_root/bin/armsx2-sdl"
...
if [[ "$bundle_sdl3_sha" != "$expected_sdl3_sha" ]]; then
	die "packaged SDL3 SHA256 mismatch: ${bundle_sdl3_sha}"
fi
...
bundle_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
binary="${bundle_dir}/bin/armsx2-sdl"
...
manifest_tmp="$(mktemp /tmp/pcsx2-sdl-r36s-manifest.XXXXXX)"
{
...
printf 'Binary SHA256: %s\n' "$(sha256_of "$bundle_root/bin/armsx2-sdl")"
